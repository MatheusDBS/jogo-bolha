...
let bubbleImg, miniBubbleImg, baiacuImg, oasisImg, heartImg, batmanImg, platformImg;
let moonImg, bossImg, popBaiacuSound, bossBulletImg, glock19Sound, batmanSound, successSound, hitpopSound, failSound;
let explosionImg, batBolhaAtirandoImg, bombaGif;
...
const player = {
  ...
  draw() {
    ...
    image(atirando ? batBolhaAtirandoImg : batmanImg, -this.radius * 1.3, -this.radius * 1.3, this.radius * 2.6, this.radius * 2.6);
    pop();
  },
...
const bossConfig = {
  init() {
    boss = { x: width - 200, y: height / 2, radius: 60, hp: 50, speed: 2, direction: 1, attackTimer: 0,
  ...
  draw() {
    ...
    fill('lime'); rect(boss.x - 50, boss.y - boss.radius - 20, 100 * (boss.hp / 50), 10);
    if (boss.attackMode === 'special') {
      push(); noFill(); stroke(255, 0, 0); strokeWeight(4);
      ellipse(boss.x, boss.y, boss.radius * 2 + 20 + sin(frameCount * 0.2) * 10);
      pop();
    }
    if (boss.bomb?.active) image(bombaGif, boss.bomb.x - boss.bomb.radius, boss.bomb.y - boss.bomb.radius, boss.bomb.radius * 2, boss.bomb.radius * 2);
  },
  resetAttack() {
...
function drawBackground() {
  ...
  platforms.forEach(plat => image(platformImg, plat.x, plat.y, plat.w, plat.h));
}

function drawOasis({ x, y }) {
  push(); translate(x, y); image(oasisImg, -16, -16, 32, 32); pop();
}
...
function drawExplosion() {
  ...
  let frameWidth = explosionImg.width / 10, frameHeight = explosionImg.height;
  image(explosionImg, explosionX - (frameWidth * 3) / 2, explosionY - (frameHeight * 3) / 2, frameWidth * 3, frameHeight * 3,
...
function showGameOverScreen() {
  gameOver = true;
  if (batmanSound?.isPlaying()) batmanSound.stop();
  failSound.play();
  ...
  window.startBtn = createButtonStyled('Iniciar Jogo', (windowWidth - 200) / 2, (windowHeight - 60) / 2 + 60, 200, 60, () => {
    ...
    if (batmanSound) {
      try {
        ...
        console.warn("Erro ao reproduzir batmanSound, provavelmente devido à política de autoplay. Interação do usuário necessária.");
      }
    }
    loop();
  });
...
function draw() {
  ...
  for (let i = 0; i < 3; i++) {
    let x = 30 + i * 35, y = 60;
    i < lives ? image(heartImg, x - 16, y - 16, 32, 32) :
      (tint(50, 50, 50, 180), image(heartImg, x - 16, y - 16, 32, 32), noTint());
  }
  if (!gameOver) {
    player.update(); player.draw(); bossConfig.update(); bossConfig.draw();
    bossBullets = bossBullets.filter(b => {
      b.x += b.dx; b.y += b.dy;
      image(bossBulletImg, b.x - b.radius, b.y - b.radius, b.radius * 2, b.radius * 2);
      if (!player.invincible && Math.hypot(player.x - b.x, player.y - b.y) < player.radius + b.radius) {
        lives--; hitpopSound.play(); player.invincible = true;
    ...
    miniBubbles = miniBubbles.filter(mb => {
      mb.x += mb.speed; image(miniBubbleImg, mb.x - mb.radius, mb.y - mb.radius, mb.radius * 2, mb.radius * 2);
    ...
    if (boss?.hp <= 0) {
      if (batmanSound?.isPlaying()) batmanSound.stop(); successSound.play();
      fill('rgba(0, 0, 0, 0.6)'); rect(0, 0, width, height); fill('#fff'); textAlign(CENTER);
      textSize(40); text('Você venceu a Fase 1!', width / 2, height / 2 - 40);
      textSize(20); text(`Pontuação final: ${score}`, width / 2, height / 2);
      text(`Vidas restantes: ${lives}`, width / 2, height / 2 + 30);
      if (!window.nextPhaseBtn) {
        window.nextPhaseBtn = createButtonStyled('Ir para Fase 2', (windowWidth - 200) / 2, windowHeight / 2 + 80, 200, 60, () => {
          (typeof trocarFase === 'function' ? trocarFase : window.parent?.trocarFase || (() => alert('Função de troca de fase não encontrada!')))('fase_2.js');
        });
