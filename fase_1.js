  ...
  if (!gameOver) {
    ...
    if (score < 10) {
      ...
      if (boss.hp <= 0) {
        ...
        textSize(40);
        textAlign(CENTER);
        text('Você venceu a Fase 1!', width / 2, height / 2 - 40);
        textSize(20);
        text(`Pontuação final: ${score}`, width / 2, height / 2);
        text(`Vidas restantes: ${lives}`, width / 2, height / 2 + 30);
        // Botão para ir para a Fase 2
        if (!window.nextPhaseBtn) {
          const nextBtn = createButton('Ir para Fase 2');
          nextBtn.size(200, 60);
          nextBtn.style('font-size', '26px');
          nextBtn.style('background', 'linear-gradient(90deg, #00c3ff 0%,#b71cff 100%)');
          nextBtn.style('color', '#fff');
          nextBtn.style('border', 'none');
          nextBtn.style('border-radius', '30px');
          nextBtn.style('box-shadow', '0 4px 16px rgba(0,0,0,0.2)');
          nextBtn.style('font-weight', 'bold');
          nextBtn.style('cursor', 'pointer');
          nextBtn.position((windowWidth - 200) / 2, (windowHeight) / 2 + 80);
          nextBtn.mousePressed(() => {
            // Troca para fase 2 no index.html
            if (typeof trocarFase === 'function') {
              trocarFase('fase_2.js');
            } else if (window.parent && typeof window.parent.trocarFase === 'function') {
              window.parent.trocarFase('fase_2.js');
            } else {
              alert('Função de troca de fase não encontrada!');
            }
          });
          window.nextPhaseBtn = nextBtn;
        } else {
          window.nextPhaseBtn.position((windowWidth - 200) / 2, (windowHeight) / 2 + 80);
          window.nextPhaseBtn.show();
        }
        if (window.restartBtn) {
